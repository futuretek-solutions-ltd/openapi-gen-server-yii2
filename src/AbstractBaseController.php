...
abstract class AbstractBaseController extends Controller
{
    ...
    /**
     * List of field/parameter names whose values are redacted in all log output.
     * Override in your controller (or a base controller) to extend the list.
     *
     * Matching is case-insensitive and exact (not a substring match).
     *
     * @var string[]
     */
    protected array $sensitiveFields = ['password'];
    ...
    public function bindActionParams($action, $params): array
    {
        ...
        foreach ($paramsMeta as $paramMeta) {
            ...
            $resolvedParams[$phpName] = $this->isSensitiveField($phpName)
                ? '[REDACTED]'
    ...
    /**
     * Returns true when the given field/parameter name should be redacted in log output.
     *
     * Comparison is case-insensitive.
     */
    private function isSensitiveField(string $name): bool
    {
        $nameLower = strtolower($name);
        foreach ($this->sensitiveFields as $sensitive) {
            if (strtolower($sensitive) === $nameLower) {
                return true;
            }
        }
        return false;
    ...
    private function summarizeData(mixed $data, int $maxItems = 5, int $maxStringLength = 200): mixed
    {
        if (is_array($data)) {
            ...
            foreach ($data as $key => $value) {
                $result[$key] = $this->isSensitiveField((string) $key)
                    ? '[REDACTED]'
                    : $this->summarizeData($value, $maxItems, $maxStringLength);
            }
            return $result;
        }
